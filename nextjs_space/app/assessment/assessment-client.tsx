// Client component for the Leadership Personas Assessment - Clean Version
'use client';
...
// 🔧 Helper: Remove any leading numbering from the statement
const cleanQuestionText = (s: string) =>
  s.replace(/^\s*\(?\d{1,3}\)?\s*[\.\):-]?\s*/, '').trim();
  ...
  useEffect(() => {
    const loadQuestions = async () => {
      try {
        ...
        } else throw new Error('Invalid questions data received');
      } catch (error) {
        console.error(error);
